! Generated by using cif2qe Version 1.2 - Date: Sun Mar 18 13:17:16 CDT 2018
!   _symmetry_space_group_name_H-M = 
!   _symmetry_Int_Tables_number = 1
!   _symmetry_cell_setting = 
! a=6.35902  b=6.35902  c=5.02422  alpha=90  beta=90  gamma=120
! Found by cif2qe: lattice = hexagonal    Space group = P1   ibrav = 4
!
!
! Symmetry found:
!   1                        x, y, z    [x]  [ y]  [ z]
&CONTROL
                       title = 'hP8-Mg3Cd-prototype.cif'
                 calculation = 'relax'
                restart_mode = 'from_scratch'
                      outdir = './1'
                  pseudo_dir = '../PP/atompaw'
                      prefix = 'hP8-Mg3Cd-prototype'
                     disk_io = 'none'
                   verbosity = 'default'
               etot_conv_thr = 0.0001
               forc_conv_thr = 0.001
                       nstep = 400
                     tstress = .true.
                     tprnfor = .true.
 /
 &SYSTEM
                       ibrav = 0
                         nat = 8
                        ntyp = 2
                     ecutwfc = 60
                     ecutrho = 480
                    vdw_corr = 'xdm'
                      xdm_a1 = 1.2153
                      xdm_a2 = 2.3704
 /
 &ELECTRONS
            electron_maxstep = 200
                    conv_thr = 1.0D-7
              diago_thr_init = 1e-4
                 startingpot = 'atomic'
                 startingwfc = 'atomic'
                 mixing_mode = 'plain'
                 mixing_beta = 0.5
                 mixing_ndim = 8
             diagonalization = 'david'
 /
&IONS
                ion_dynamics = 'bfgs'
 /

ATOMIC_SPECIES
   Mg   24.3050000000  Mg.pw86pbe-n-kjpaw_psl.1.0.0.UPF
   Cd  112.4110000000  Cd.pw86pbe-n-kjpaw_psl.1.0.0.UPF

ATOMIC_POSITIONS crystal
Mg      0.66385800000000      0.83192900000000      0.25000000000000
Mg      0.83192900000000      0.16807100000000      0.75000000000000
Mg      0.33614200000000      0.16807100000000      0.75000000000000
Mg      0.16807100000000      0.83192900000000      0.25000000000000
Mg      0.16807100000000      0.33614200000000      0.25000000000000
Mg      0.83192900000000      0.66385800000000      0.75000000000000
Cd      0.66666700000000      0.33333300000000      0.25000000000000
Cd      0.33333300000000      0.66666700000000      0.75000000000000

K_POINTS automatic
2  2  3   0 0 0

CELL_PARAMETERS
    12.01680126954927      0.00000000000000      0.00000000000000
    -6.00840063477463     10.40685517165877      0.00000000000000
     0.00000000000000      0.00000000000000      9.49439268419205



